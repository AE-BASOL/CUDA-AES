...
static __device__ __forceinline__ void aes128_ofb_crypt_block_chain(const uint8_t *in, uint8_t *out, size_t nBlocks, const uint8_t *iv) {
    ...
    for (size_t blk = 0; blk < nBlocks; ++blk) {
        aes_encrypt_block(feedback, rk, 10);
#pragma unroll
        for (int i = 0; i < 16; ++i) {
...
__global__ void aes128_ofb_encrypt(const uint8_t *in, uint8_t *out, size_t nBlocks, const uint8_t *iv) {
    aes128_ofb_crypt_block_chain(in, out, nBlocks, iv);
}
