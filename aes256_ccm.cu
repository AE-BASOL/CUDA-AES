...
__device__ __forceinline__ void aes256_encrypt_block(const uint8_t in[16], uint8_t out[16]) {
...
#pragma unroll
    ...
    store_le32(out, k0 ^ rk[56]);
    store_le32(out + 4, k1 ^ rk[57]);
    store_le32(out + 8, k2 ^ rk[58]);
    store_le32(out + 12, k3 ^ rk[59]);
}
...
#pragma unroll
    ...
    aes256_encrypt_block(block, encrypted);
#pragma unroll
...
__global__ void aes256_ccm_encrypt(const uint8_t *plain, uint8_t *cipher, size_t nBlocks, const uint8_t *nonce, uint8_t *tagOut) {
    ...
    for (size_t block = threadIdx.x; block < nBlocks; block += blockDim.x) {
        ...
        aes256_encrypt_block(counter, stream);
#pragma unroll
